...
use App\Http\Controllers\Auth\AuthenticatedSessionController as AdminAuthenticateController;
use App\Http\Controllers\Admin\Auth\ConfirmablePasswordController as AdminConfirmablePasswordController;
use App\Http\Controllers\Admin\Auth\EmailVerificationNotificationController as AdminNotiController;
use App\Http\Controllers\Admin\Auth\EmailVerificationPromptController as AdminEmailController;
...
use App\Http\Controllers\Admin\Auth\PasswordResetLinkController as AdminPasswordResetController;
use App\Http\Controllers\Admin\Auth\RegisteredUserController as AdminRegisterController;
use App\Http\Controllers\Admin\Auth\VerifyEmailController as AdminVerifyController;
...
Route::middleware('guest:admin')->group(function () {
    ...
    Route::get('/admin/login', [AdminAuthenticateController::class, 'create'])
                ->name('admin.login');

    Route::post('/admin/login', [AdminAuthenticateController::class, 'store']);
...
Route::middleware('admin')->name('admin.')->prefix('admin')->group(function(){
    Route::get('verify-email', [AdminEmailController::class, '__invoke'])
                ->name('verification.notice');

    Route::get('verify-email/{id}/{hash}', [AdminVerifyController::class, '__invoke'])
                ->middleware(['signed', 'throttle:6,1'])
                ->name('verification.verify');

    Route::post('email/verification-notification', [AdminNotiController::class, 'store'])
                ->middleware('throttle:6,1')
                ->name('verification.send');
