  ...
  <div class="py-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="flex m-2 p-2">
              <a href="{{ route('member.bookings.index') }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Reservation Index</a>
          </div>

          <div class="m-2 p-2 bg-slate-100 rounded">
              <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                  <form method="POST" action="{{ route('member.bookings.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    {{-- <div class="sm:col-span-6">
                      ...
                      <div class="mt-1">
                        <input type="text" id="club" name="club" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                      </div>
                      @error('club')
                        <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="sm:col-span-6 pt-5">
                      ...
                      <div class="mt-1">
                        <textarea id="purpose" rows="3" name="purpose" class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"></textarea>
                      </div>
                      @error('purpose')
                        <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                    ...
                    <br>
                      ...
                      <div class="sm:col-span-6">
                        ...
                        <div class="mt-1">
                          <input type="datetime-local" id="booking_date" name="booking_date" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                        </div>
                        @error('booking_date')
                        <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
